...
class ResourceMixin(models.AbstractModel):
    ...
    def _get_work_days_data(self, from_datetime, to_datetime, compute_leaves=True, calendar=None, domain=None):
        ...
        # --- FIX START ---
        # If no calendar is found for the employee/resource, return zeroed data
        # instead of crashing with "Expected singleton"
        if not calendar:
        ...
        # This is where the crash was happening because 'calendar' was empty
        ...
        # Added a check (day_total[day]) to prevent DivisionByZero just in case
        days = sum(
            float_utils.round(ROUNDING_FACTOR * day_hours[day] / (day_total[day] or 1.0)) / ROUNDING_FACTOR
