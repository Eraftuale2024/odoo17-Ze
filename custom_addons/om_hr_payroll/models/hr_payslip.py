...
class HrPayslip(models.Model):
    ...
    month_in_word = fields.Char(
        string="Month",  compute="compute_payroll_month", store=True)
    ...
    def action_payslip_done(self):
        ...
        # Compute sheets in batch
        self.compute_sheet()

        # Update state first
        self.write({'state': 'done'})

        # Send emails in background (optional)
        for payslip in self:
            payslip._send_payslip_email()

    def _send_payslip_email(self):
        """Send email for individual payslip (can be called separately)"""
    ...
    def refund_sheet(self):
        ...
        form_view_ref = self.env.ref(
            'om_om_hr_payroll.view_hr_payslip_form', False)
        tree_view_ref = self.env.ref(
            'om_om_hr_payroll.view_hr_payslip_tree', False)
        return {
    ...
    def action_send_email(self):
        ...
        # Use Odoo's standard email composition method with default_res_ids
        return {
            'type': 'ir.actions.act_window',
            ...
            'context': {
                ...
                'default_res_ids': [self.id],  # Changed to plural, expects list
...
class HrPayslipWorkedDays(models.Model):
    ...
    working_days = fields.Float(
        string='Worked Days', compute="compute_payslip_working_days")
    working_hours = fields.Float(
        string='Worked Hours', compute="compute_payslip_working_days")
...
class HrPayslipRun(models.Model):
    ...
    def done_payslip_run(self):
        self.ensure_one()
        slips = self.slip_ids.filtered(lambda s: s.contract_id and s.struct_id)

        if not slips:
            return

        journal = slips[0].journal_id
        date = slips[0].date or slips[0].date_to
        currency = slips[0].company_id.currency_id

        # Group lines: key = (account_id, is_debit, rule_name), value = total amount
        grouped_lines = {}
        ...
        # Build journal lines
        for (account_id, is_debit, rule_name), amount in grouped_lines.items():
            if currency.is_zero(amount):
                continue

            line_vals = {
                'name': f'Aggregated {rule_name}',
        ...
        # Handle adjustment to balance the move
        if currency.compare_amounts(debit_sum, credit_sum) == -1:
            # Credit is lower → Add extra credit
            acc_id = journal.default_account_id.id
            if not acc_id:
                raise UserError(
                    _('The journal "%s" must have a default account.') % journal.name)
            line_ids.append((0, 0, {
        ...
        elif currency.compare_amounts(credit_sum, debit_sum) == -1:
            # Debit is lower → Add extra debit
            acc_id = journal.default_account_id.id
            if not acc_id:
                raise UserError(
                    _('The journal "%s" must have a default account.') % journal.name)
        ...
        move = self.env['account.move'].create({
            'narration': _('Aggregated Payslip Batch Entry'),
            'ref': ', '.join(slips.mapped('number')),
            'journal_id': journal.id,
            'date': date,
            'line_ids': line_ids,
        })
    ...
    def unlink(self):
        for rec in self:
            if rec.state == 'done':
                raise ValidationError(
                    _('You Cannot Delete Done Payslips Batches'))
